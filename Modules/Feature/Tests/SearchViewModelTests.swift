...
@Suite struct SearchViewModelTests {
    ...
    @Test func 검색_성공시_loaded_상태가_된다() async {
        let book = sampleBook
        let vm = SearchViewModel(bookSearch: StubBookSearchRepository { _ in [book] })
        vm.keyword = "하얼빈"
    ...
    @Test func 결과가_0건이면_empty_상태가_된다() async {
        let vm = SearchViewModel(bookSearch: StubBookSearchRepository { _ in [] })
        vm.keyword = "존재하지않는책"
    ...
    @Test func 검색이_실패하면_failed_상태가_된다() async {
        let vm = SearchViewModel(bookSearch: StubBookSearchRepository { _ in throw StubError() })
        vm.keyword = "하얼빈"
    ...
    @Test func 검색어_앞뒤_공백은_제거되어_전달된다() async {
        ...
        let vm = SearchViewModel(bookSearch: StubBookSearchRepository { keyword in
